(function () {
    ...
    function temperatureClass(value) {
        ...
        if (value >= 35) { return 'icong-temp-extreme'; }
        if (value >= 30) { return 'icong-temp-hot'; }
        if (value >= 22) { return 'icong-temp-warm'; }
        if (value >= 12) { return 'icong-temp-mild'; }
        if (value >= 4) { return 'icong-temp-cool'; }
        return 'icong-temp-cold';
    }

    function gustClass(value) {
        if (!finite(value)) { return ''; }
        if (value >= 80) { return 'icong-gust-danger'; }
        if (value >= 60) { return 'icong-gust-strong'; }
        if (value >= 40) { return 'icong-gust-moderate'; }
        return '';
    }

    function windForceClass(value) {
        if (!finite(value)) { return 'icong-wind-calm'; }
        if (value >= 80) { return 'icong-wind-violent'; }
        if (value >= 60) { return 'icong-wind-strong'; }
        if (value >= 40) { return 'icong-wind-fresh'; }
        if (value >= 20) { return 'icong-wind-moderate'; }
        return 'icong-wind-light';
    }

    function rainClass(value) {
        if (!finite(value) || value < 0.1) { return ''; }
        if (value >= 15) { return 'icong-rain icong-rain-intense'; }
        if (value >= 5) { return 'icong-rain icong-rain-strong'; }
        if (value >= 2) { return 'icong-rain icong-rain-moderate'; }
        if (value >= 0.5) { return 'icong-rain icong-rain-light'; }
        return 'icong-rain icong-rain-trace';
    }
    ...
    function chartDayBands(svg, count, margin, innerWidth, innerHeight, meta) {
        ...
        while (start < count) {
            ...
            svg.appendChild(svgElement('rect', {
                ...
                height: innerHeight,
                class: 'icong-chart-day-band icong-dayband-' + token
            }));
            ...
            svg.appendChild(svgElement('rect', {
                ...
                width: Math.max(0, endX - startX),
                height: 46,
                class: 'icong-chart-day-strip icong-daystrip-' + token
            }));
            ...
            if (start > 0) {
                svg.appendChild(svgElement('line', {
                    ...
                    class: 'icong-chart-day-separator icong-dayseparator-' + token
                }));
            }
            ...
            if (endX - startX > 72) {
                ...
                svg.appendChild(svgElement('text', {
                    ...
                    y: margin.top + 31,
                    'text-anchor': 'middle',
                    class: 'icong-chart-day-label icong-daylabel-' + token
                }, dayLabel));
            }
            start = end + 1;
        }
    ...
    function appendChartMetrics(container, metrics) {
        ...
        block.className = 'icong-chart-metrics';
        valid.forEach(function (metric) {
            var item = document.createElement('span');
            item.className = 'icong-chart-metric';
            item.appendChild(textSpan(metric.label, 'icong-chart-metric-label'));
            item.appendChild(textSpan(metric.value, 'icong-chart-metric-value'));
    ...
    function appendChartViewport(container, svg) {
        ...
        viewport.className = 'icong-chart-viewport';
        var tooltip = document.createElement('div');
        tooltip.className = 'icong-chart-tooltip';
        tooltip.hidden = true;
        ...
        function tooltipTarget(node) {
            ...
            return node.closest('[data-icong-tooltip]');
        }
        ...
        viewport.addEventListener('pointerover', function (event) {
            ...
            if (!target) { return; }
            tooltip.textContent = target.getAttribute('data-icong-tooltip') || '';
    ...
    function drawExtremaLabels(svg, points, values, formatter, className) {
        ...
        indexes.forEach(function (index) {
            ...
            svg.appendChild(svgElement('text', {
                ...
                y: y,
                'text-anchor': 'middle',
                class: 'icong-chart-value-label ' + className
    ...
    function renderLineChart(container, labels, series, options) {
        ...
        if (!scale) {
            container.appendChild(textSpan('Données indisponibles pour ce diagramme.', 'icong-chart-empty'));
            return;
        }
        ...
        svg.classList.add('icong-chart-svg');
        ...
        for (var g = 0; g <= tickValue; g += 1) {
            ...
            svg.appendChild(svgElement('line', {
                ...
                y1: gy,
                y2: gy,
                class: 'icong-chart-grid'
            }));
            svg.appendChild(svgElement('text', {
                x: margin.left - 9,
                y: gy + 4,
                'text-anchor': 'end',
                class: 'icong-chart-axis'
        ...
        chartTickIndexes(n, 8).forEach(function (index) {
            ...
            svg.appendChild(svgElement('line', {
                ...
                y1: margin.top,
                y2: margin.top + innerHeight,
                class: 'icong-chart-grid icong-chart-grid-vertical'
            }));
            svg.appendChild(svgElement('text', {
                x: x,
                y: height - 18,
                'text-anchor': index === 0 ? 'start' : (index === n - 1 ? 'end' : 'middle'),
                class: 'icong-chart-axis icong-chart-xaxis'
            }, axisLabel));
        });
        ...
        series.forEach(function (item, seriesIndex) {
            ...
            if (series.length === 1 && options.fillArea) {
                segments.forEach(function (segment) {
                    ...
                    svg.appendChild(svgElement('path', {
                        d: d,
                        class: 'icong-chart-area ' + item.className
                    }));
                });
            }
            ...
            segments.forEach(function (segment) {
                svg.appendChild(svgElement('path', {
                    ...
                    class: 'icong-chart-line ' + item.className,
                    fill: 'none'
                }));
            });
            ...
            points.forEach(function (point, index) {
                ...
                var attrs = {
                    ...
                    class: (options.showPoints === false ? 'icong-chart-hitpoint ' : 'icong-chart-point ') + item.className,
                    tabindex: 0,
                    'aria-label': tooltipLabel,
                    'data-icong-tooltip': tooltipLabel
                };
        ...
        if (options.bandBetween && seriesPoints.length >= 2) {
            ...
            if (upperPoints.length > 1 && upperPoints.length === lowerPoints.length) {
                ...
                var envelope = svgElement('path', {
                    d: area,
                    class: 'icong-chart-wind-envelope'
                });
                var firstLine = svg.querySelector('.icong-chart-line');
        ...
        if (series.length > 1) {
            var legend = svgElement('g', { class: 'icong-chart-legend' });
            series.forEach(function (item, index) {
                ...
                legend.appendChild(svgElement('text', {
                    x: x + 36,
                    y: 21,
                    class: 'icong-chart-axis icong-chart-legend-text'
                }, item.label));
            });
    ...
    function rainVisualClass(value) {
        if (!finite(value) || value < 0.1) { return 'icong-chart-rain-zero'; }
        if (value >= 15) { return 'icong-chart-rain-intense'; }
        if (value >= 5) { return 'icong-chart-rain-strong'; }
        if (value >= 2) { return 'icong-chart-rain-moderate'; }
        if (value >= 0.5) { return 'icong-chart-rain-light'; }
        return 'icong-chart-rain-trace';
    }
    ...
    function renderRainChart(container, labels, rainValues, cumulativeValues, options) {
        ...
        for (var cg = 0; cg <= cumTickCount; cg += 1) {
            ...
            svg.appendChild(svgElement('text', {
                ...
                y: cy + 5,
                'text-anchor': 'start',
                class: 'icong-chart-axis icong-chart-axis-cumulative'
        ...
        rainValues.forEach(function (value, index) {
            ...
            var bar = svgElement('rect', {
                ...
                class: 'icong-chart-rain-bar ' + rainVisualClass(value),
                tabindex: 0,
                'aria-label': rainTooltip,
                'data-icong-tooltip': rainTooltip
            });
            svg.appendChild(bar);
        });
        ...
        if (cleanPoints.length) {
            svg.appendChild(svgElement('path', {
                ...
                class: 'icong-chart-line icong-series-rain-cumulative',
                fill: 'none'
            }));
            cumulativePoints.forEach(function (point, index) {
                ...
                var hitPoint = svgElement('circle', {
                    ...
                    cy: point.y,
                    r: 11,
                    class: 'icong-chart-hitpoint icong-series-rain-cumulative',
                    tabindex: 0,
                    'aria-label': cumulativeTooltip,
                    'data-icong-tooltip': cumulativeTooltip,
                    fill: 'transparent',
                    stroke: 'none',
                    opacity: '0'
                });
        ...
        legend.appendChild(svgElement('rect', {
            ...
            height: 11,
            rx: 4,
            class: 'icong-chart-rain-bar icong-chart-rain-light'
        }));
        legend.appendChild(svgElement('text', {
            x: margin.left + 28,
            y: 21,
    ...
    function initApp(app) {
        ...
        var hours = Math.max(1, Math.min(180, parseInt(app.dataset.hours || '180', 10)));
        var timezone = app.dataset.timezone || 'Europe/Paris';
        var titlePrefix = app.dataset.titlePrefix || 'Prévisions ICON-GLOBAL';

        var input = app.querySelector('.icong-city-input');
        var locateButton = app.querySelector('[data-icong-locate]');
        var results = app.querySelector('.icong-search-results');
        var status = app.querySelector('.icong-search-status');
        var generalBody = app.querySelector('[data-icong-body-general]');
        var stormBody = app.querySelector('[data-icong-body-storms]');
        var stormSummary = app.querySelector('[data-icong-storm-summary]');
        var snowBody = app.querySelector('[data-icong-body-snow]');
        var snowSummary = app.querySelector('[data-icong-snow-summary]');
        var tabs = Array.prototype.slice.call(app.querySelectorAll('[data-icong-tab]'));
        var panels = Array.prototype.slice.call(app.querySelectorAll('[data-icong-panel]'));
        var title = app.querySelector('[data-icong-title]');
        var altitudeLine = app.querySelector('[data-icong-altitude]');
        var meta = app.querySelector('[data-icong-meta]');
        var generated = app.querySelector('[data-icong-generated]');
        var stale = app.querySelector('[data-icong-stale]');
        var stormTopScroll = app.querySelector('[data-icong-top-scroll="storms"]');
        var stormScrollWrap = app.querySelector('[data-icong-scroll-wrap="storms"]');
        var stormTable = app.querySelector('.icong-storm-table');
        var snowTopScroll = app.querySelector('[data-icong-top-scroll="snow"]');
        var snowScrollWrap = app.querySelector('[data-icong-scroll-wrap="snow"]');
        var snowTable = app.querySelector('.icong-snow-table');
        var chartTemperature = app.querySelector('[data-icong-chart-temperature]');
        var chartPressure = app.querySelector('[data-icong-chart-pressure]');
        var chartRain = app.querySelector('[data-icong-chart-rain]');
        var chartWind = app.querySelector('[data-icong-chart-wind]');
        var chartTitleTemperature = app.querySelector('[data-icong-chart-title-temperature]');
        var chartTitlePressure = app.querySelector('[data-icong-chart-title-pressure]');
        var chartTitleRain = app.querySelector('[data-icong-chart-title-rain]');
        var chartTitleWind = app.querySelector('[data-icong-chart-title-wind]');
        var rainTotal = app.querySelector('[data-icong-rain-total]');
        ...
        function makeDayCell(date, rowSpan) {
            var cell = createCell('th', 'Date', 'icong-day-cell icong-day-' + weekdayToken(date));
            cell.scope = 'rowgroup';
            cell.rowSpan = rowSpan;
            cell.appendChild(textSpan(weekdayFormat.format(date), 'icong-day-weekday'));
            cell.appendChild(textSpan(shortDateFormat.format(date), 'icong-day-date'));
            return cell;
        }
        ...
        function setStatus(message, error) {
            ...
            status.classList.toggle('icong-search-error', Boolean(error));
        }

        function setActiveView(view) {
            tabs.forEach(function (button) {
                var active = button.dataset.icongTab === view;
                button.classList.toggle('is-active', active);
                button.setAttribute('aria-selected', active ? 'true' : 'false');
            });
            panels.forEach(function (panel) {
                panel.hidden = panel.dataset.icongPanel !== view;
            });
            ...
            if (view === 'map' && selectedMapFocus) {
                window.requestAnimationFrame(function () {
                    var mapApp = app.querySelector('[data-icongm-app]');
                    if (mapApp) {
                        mapApp.dispatchEvent(new CustomEvent('icongm:focus-location', {
        ...
        function putMessage(body, message, error, colspan) {
            ...
            var cell = createCell('td', '', error ? 'icong-loading icong-load-error' : 'icong-loading');
        ...
        function showTableMessage(message, error) {
            ...
            if (stormSummary) {
                ...
                stormSummary.classList.toggle('icong-storm-summary-warning', Boolean(error));
            }
            if (snowSummary) {
                snowSummary.textContent = message;
                snowSummary.classList.toggle('icong-snow-summary-warning', Boolean(error));
            }
        }
            ...
            return fetchJson(baseUrl + '/index.json', { cache: 'no-cache' })
                .then(function (payload) {
                    if (!payload || payload.status !== 'ok' || !payload.departments) {
                        throw new Error('Index national ICON-GLOBAL invalide');
                    }
                    indexData = payload;
                    ...
                    meta.textContent = 'Run du ' + run + ' • résolution ' + resolution +
                        ((payload.model && payload.model.storm_diagnostics) ? ' • diagnostics orages + neige ICON-GLOBAL' : '');
        ...
        function loadDepartment(code) {
            ...
            if (!indexData || !indexData.departments[normalized]) {
                return Promise.reject(new Error('Département hors couverture ICON-GLOBAL'));
            }
        ...
        function renderForecast(departmentData, commune) {
            ...
            function appendHazard(row, label, code) {
                ...
                var cell = createCell('td', label, numeric === null ? '' : 'icong-hazard icong-hazard-' + numeric);
            ...
            if (!forecasts.length) {
                showTableMessage('Aucune échéance ICON-GLOBAL disponible pour cette commune.', true);
                return;
            }
            ...
            forecasts.forEach(function (step, currentIndex) {
                ...
                if (dayKey !== previousGeneralDay) {
                    row.classList.add('icong-new-day');
                    row.appendChild(makeDayCell(date, dayCounts[dayKey]));
                    previousGeneralDay = dayKey;
                }

                var hourCell = createCell('td', 'Heure', 'icong-hour icong-hour-' + weekdayToken(date));
                ...
                var conditionCell = createCell('td', 'Temps', 'icong-condition');
                conditionCell.appendChild(textSpan(condition.icon, 'icong-icon'));
                conditionCell.appendChild(textSpan(condition.label));
                row.appendChild(conditionCell);

                var temperatureCell = createCell('td', 'Température', 'icong-temperature ' + temperatureClass(temp));
                ...
                var windCell = createCell('td', 'Vent', 'icong-wind-cell ' + windForceClass(windDisplay));
                ...
                if (dayKey !== previousStormDay) {
                    stormRow.classList.add('icong-new-day');
                    stormRow.appendChild(makeDayCell(date, dayCounts[dayKey]));
                    previousStormDay = dayKey;
                }
                var stormHour = createCell('td', 'Heure', 'icong-hour icong-hour-' + weekdayToken(date));
                ...
                var thunderCell = createCell('td', 'Risque orage', thunder ? 'icong-thunder-risk icong-thunder-' + Number(thunderCode) : '');
                if (thunder) {
                    thunderCell.appendChild(textSpan(thunder.icon + ' ', 'icong-risk-icon'));
                    thunderCell.appendChild(textSpan(thunder.label, 'icong-risk-label'));
                } else {
                ...
                var capeCell = createCell('td', 'CAPE', finite(cape) && cape >= 1500 ? 'icong-cape-high' : (finite(cape) && cape >= 500 ? 'icong-cape-mid' : ''));
                capeCell.textContent = finite(cape) && Number(cape) >= 25 ? formatNumber(cape, 0) + ' J/kg' : '—';
                capeCell.title = 'CAPE instantanée directement produite par ICON-GLOBAL ; les valeurs quasi nulles ne sont pas affichées.';
                stormRow.appendChild(capeCell);
                appendNumber(stormRow, 'LCL', value(values, 'lcl_m'), 0, ' m');

                var lightning = value(values, 'lightning_score');
                appendNumber(stormRow, 'Foudre', lightning, 0, '/100', finite(lightning) && lightning >= 60 ? 'icong-lightning-high' : '');
                ...
                var stormTypeCell = createCell('td', 'Type d’orage', 'icong-storm-type');
                ...
                var detailsCell = createCell('td', 'Détails', 'icong-details-cell');
                var detailsButton = document.createElement('button');
                detailsButton.type = 'button';
                detailsButton.className = 'icong-details-button';
                detailsButton.textContent = 'Afficher';
                detailsButton.setAttribute('aria-expanded', 'false');
                detailsCell.appendChild(detailsButton);

                var advanced = document.createElement('div');
                advanced.className = 'icong-advanced-list icong-details-inline';
                advanced.hidden = true;
                [
                    ['CAPE directe', value(values, 'cape_jkg'), 0, ' J/kg'],
                    ...
                    box.className = 'icong-advanced-item';
                    box.appendChild(textSpan(item[0], 'icong-advanced-label'));
                    box.appendChild(textSpan(finite(item[1]) ? formatNumber(item[1], item[2]) + item[3] : '—', 'icong-advanced-value'));
                ...
                if (dayKey !== previousSnowDay) {
                    snowRow.classList.add('icong-new-day');
                    snowRow.appendChild(makeDayCell(date, dayCounts[dayKey]));
                    previousSnowDay = dayKey;
                }
                var snowHour = createCell('td', 'Heure', 'icong-hour icong-hour-' + weekdayToken(date));
                ...
                var snowRiskCell = createCell('td', 'Risque neige', snowRisk ? 'icong-snow-risk icong-snow-' + Number(snowRiskCode) : '');
                if (snowRisk) {
                    snowRiskCell.appendChild(textSpan(snowRisk.icon + ' ', 'icong-risk-icon'));
                    snowRiskCell.appendChild(textSpan(snowRisk.label, 'icong-risk-label'));
                ...
                var stickCell = createCell('td', 'Tenue', finite(stickCode) ? 'icong-stick icong-stick-' + Number(stickCode) : '');
                ...
                var snowWindCell = createCell('td', 'Vent moyen / rafales', 'icong-snow-wind ' + windForceClass(gustDisplay));
                ...
                var snowDetailsCell = createCell('td', 'Détails', 'icong-details-cell icong-snow-details-cell');
                var snowDetailsButton = document.createElement('button');
                snowDetailsButton.type = 'button';
                snowDetailsButton.className = 'icong-details-button';
                ...
                snowAdvanced.className = 'icong-snow-profile icong-details-inline';
                snowAdvanced.hidden = true;

                function appendProfileSection(titleText, items) {
                    var section = document.createElement('section');
                    section.className = 'icong-profile-section';
                    section.appendChild(textSpan(titleText, 'icong-profile-title'));
                    var list = document.createElement('div');
                    list.className = 'icong-profile-list';
                    items.forEach(function (item) {
                        var profileItem = document.createElement('div');
                        profileItem.className = 'icong-profile-item';
                        profileItem.appendChild(textSpan(item[0], 'icong-profile-label'));
                        profileItem.appendChild(textSpan(finite(item[1]) ? formatNumber(item[1], item[2]) + item[3] : '—', 'icong-profile-value'));
                ...
                appendProfileSection('Sorties et diagnostics ICON-GLOBAL', [
            ...
            if (stormSummary) {
                stormSummary.classList.toggle('icong-storm-summary-warning', !hasStormColumns);
                if (!hasStormColumns) {
                    ...
                    stormSummary.textContent = 'Risque orageux maximal sur les ' + forecasts.length + ' prochaines échéances : ' + maxRisk.icon + ' ' + maxRisk.label + ' • CAPE ICON-GLOBAL : risque indicatif, réflectivité indisponible.';
                }
            }
            if (snowSummary) {
                snowSummary.classList.toggle('icong-snow-summary-warning', !hasSnowColumns);
                if (!hasSnowColumns) {
                    ...
                    snowSummary.textContent = 'Risque de neige maximal sur les ' + forecasts.length + ' prochaines échéances : ' + maxSnow.icon + ' ' + maxSnow.label + ' • cumul neige ICON-GLOBAL 13 km.';
                }
            }
            ...
            renderLineChart(chartTemperature, chartLabels, [{ label: 'Température', values: chartTemps, className: 'icong-series-temperature' }], {
                decimals: 0,
                unit: '°C',
                fillArea: false,
            ...
            renderLineChart(chartPressure, chartLabels, [{ label: 'Pression', values: chartPressures, className: 'icong-series-pressure' }], {
                decimals: 0,
                unit: 'hPa',
                fillArea: false,
                metricMode: 'pressure',
            ...
            renderLineChart(chartWind, chartLabels, [
                { label: 'Vent moyen', values: chartWindValues, className: 'icong-series-wind' },
                { label: 'Rafales', values: chartGustValues, className: 'icong-series-gust' }
            ], {
                decimals: 0,
                unit: 'km/h',
                forceZero: true,
        ...
        function selectCommune(candidate) {
            ...
            loadDepartment(candidate.codeDepartement)
                .then(function (departmentData) {
                    ...
                    if (!commune) {
                        throw new Error('Commune absente du catalogue ICON-GLOBAL');
                    }
        ...
        function displaySearchResults(candidates) {
            ...
            candidates.forEach(function (candidate, position) {
                ...
                button.type = 'button';
                button.className = 'icong-search-result';
                button.id = input.id + '-option-' + position;
                button.setAttribute('role', 'option');
                button.setAttribute('aria-selected', 'false');

                var name = textSpan(candidate.nom, 'icong-result-name');
        ...
        if (!baseUrl) {
            showTableMessage('Adresse des données ICON-GLOBAL non configurée.', true);
            return;
        }
        loadIndex()
            .then(function () {
    ...
    whenReady(function () {
        document.querySelectorAll('[data-icong-app]').forEach(initApp);
    });
}());
